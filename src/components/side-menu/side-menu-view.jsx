...
function ResponsiveDrawer(props) {
  ...
  const formatCategories = async (data) => {
    ...
    const formattedMainCategories = await Promise.all(
  ...
  const generateMenuItems = (children, startIndex = 0) => {
    ...
    loopElements.forEach((category, i) => {
      if (category.children.length === 0) {
        ...
        menuItems.push(
          <span key={i}>
            <ListItem button onClick={() => handleClick(i + startIndex, category.to)}>
              <ListItemText primary={category.category} />
              {nestedOpen[i + startIndex ] ? <ExpandLess /> : <ExpandMore />}
            </ListItem>
            <Collapse
              in={nestedOpen[i + startIndex]}
  ...
  const drawer = (
    <div>
      ...
      <List
        ...
        className={classes.listRoot}
      >
        {props.signedIn && generateMenuItems(renderData, adminCategories.length)}
      </List>
    </div>
  );
